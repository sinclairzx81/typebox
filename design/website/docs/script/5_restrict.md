...
TypeBox provides a `maxInstantiationCount` configuration which is available via [System Settings](/#/docs/system/1_settings). This configuration can be overridden to allow higher instantiation counts if necessary, or set to `0` to disable generic calls entirely.
