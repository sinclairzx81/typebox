...
describe('value/hash/Hash', () => {
  it('Should hash number', () => {
    Assert.isEqual('bigint', typeof ValueHash.Create(1))
    const A = ValueHash.Create(1)
    const B = ValueHash.Create(2)
    Assert.notEqual(A, B)
  })
  it('Should hash string', () => {
    Assert.isEqual('bigint', typeof ValueHash.Create('hello'))
    const A = ValueHash.Create('hello')
    const B = ValueHash.Create('world')
    Assert.notEqual(A, B)
  })
  it('Should hash boolean', () => {
    Assert.isEqual('bigint', typeof ValueHash.Create(true))
    Assert.isEqual('bigint', typeof ValueHash.Create(false))
    const A = ValueHash.Create(true)
    const B = ValueHash.Create(false)
    Assert.notEqual(A, B)
  })
  it('Should hash null', () => {
    Assert.isEqual('bigint', typeof ValueHash.Create(null))
    const A = ValueHash.Create(null)
    const B = ValueHash.Create(undefined)
    Assert.notEqual(A, B)
  })
  it('Should hash array', () => {
    Assert.isEqual('bigint', typeof ValueHash.Create([0, 1, 2, 3]))
  ...
  it('Should hash object 1', () => {
    // prettier-ignore
    Assert.isEqual('bigint', typeof ValueHash.Create({ x: 1, y: 2 }))
  ...
  it('Should hash bigint 1', () => {
    Assert.isEqual('bigint', typeof ValueHash.Create(BigInt(1)))
