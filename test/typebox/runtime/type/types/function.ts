...
Test('Should Create Function 1', () => {
  const T: Type.TFunction<[Type.TNull], Type.TNull> = 
    Type.Function([Type.Null()], Type.Null())
...
Test('Should Create Function 2 (No Rest Spread)', () => {
  const T: Type.TFunction<[Type.TNull, Type.TRest<Type.TArray<Type.TNumber>>], Type.TNull> = 
    Type.Function([Type.Null(), Type.Rest(Type.Array(Type.Number()))], Type.Null())
  Assert.IsTrue(Type.IsFunction(T))
  Assert.IsTrue(Type.IsNull(T.parameters[0]))
  Assert.IsTrue(Type.IsNull(T.returnType))
})
Test('Should Create Function with options', () => {
  const T: Type.TFunction<[Type.TNull], Type.TNull> = 
    Type.Function([Type.Null()], Type.Null(), { a: 1, b: 2 })
