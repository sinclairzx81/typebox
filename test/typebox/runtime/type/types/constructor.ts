...
Test('Should Create Constructor 1', () => {
  const T: Type.TConstructor<[Type.TNull], Type.TNull> = Type.Constructor([Type.Null()], Type.Null())
...
Test('Should Create Constructor 2 (No Rest Spread)', () => {
  const T: Type.TConstructor<[Type.TNull, Type.TRest<Type.TArray<Type.TNumber>>], Type.TNull> = Type.Constructor([Type.Null(), Type.Rest(Type.Array(Type.Number()))], Type.Null())
  Assert.IsTrue(Type.IsConstructor(T))
  Assert.IsTrue(Type.IsNull(T.parameters[0]))
  Assert.IsTrue(Type.IsNull(T.instanceType))
})
Test('Should Create Constructor with options', () => {
  const T: Type.TConstructor<[Type.TNull], Type.TNull> = Type.Constructor([Type.Null()], Type.Null(), { a: 1, b: 2 })
