...
Test('Should Iterator 3', () => {
  const T: Type.TWith<Type.TIterator<Type.TNull>, {
    a: 1
    b: 2
  }> = Type.Script('Iterator<null> with { a: 1, b: 2 }')
...
Test('Should Iterator 4', () => {
  const T: Type.TIterator<Type.TNull> = Type.Script('Iterator<null> with { a: 1, b: 2 }')
