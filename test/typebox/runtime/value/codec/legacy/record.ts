...
const Test = Assert.Context('Value.Codec.Legacy.Record')
...
Test('Should Record 3', () => {
  ...
  const D = Decode(Augment, { x: 1, y: 2 })
  const E = Encode(Augment, D)
...
Test('Should Record 5', () => {
  const NumberToString = Type.Codec(Type.Number())
    ...
    .Encode((value) => parseFloat(value))
  const A = Type.Record(Type.Number(), NumberToString)
  Assert.Throws(() => Decode(A, null))
  Assert.Throws(() => Encode(A, null))
})
// ------------------------------------------------------------------
// Coverage
// ------------------------------------------------------------------
