...
const Test = Assert.Context('Value.Codec.Legacy.Intersect')
...
Test('Should Intersect 12', () => {
  ...
  const D = Decode(Outer, { x: 1, y: 2 })
  const E = Encode(Outer, D)
  Assert.IsEqual(D, { x: '1', y: '2' })
  Assert.IsEqual(E, { x: 1, y: 2 })
})
