...
Test('Should Union 1', () => {
  ...
  const D = Value.PipelineDecode(T, null)
...
Test('Should Union 2', () => {
  const T = Type.Union([Type.Null(), NumberToString])
  const D = Value.PipelineDecode(T, 100)
  const E = Value.PipelineEncode(T, D)
...
Test('Should Union 7', () => {
  const D = Value.PipelineDecode(MultiVariant, 'hello')
  const E = Value.PipelineEncode(MultiVariant, D)
  Assert.IsEqual(D, 'string')
  Assert.IsEqual(E, 'string')
})
Test('Should Union 8', () => {
  const D = Value.PipelineDecode(MultiVariant, 1)
  const E = Value.PipelineEncode(MultiVariant, D)
  Assert.IsEqual(D, 'number')
  Assert.IsEqual(E, 'string')
})
Test('Should Union 9', () => {
  const D = Value.PipelineDecode(MultiVariant, true)
  const E = Value.PipelineEncode(MultiVariant, D)
...
Test('Should Union 10', () => {
  const Identity = Type.Codec(Type.Union([
    ...
    .Decode((value) => value)
    .Encode((value) => value)
  Assert.Throws(() => Value.PipelineDecode(Identity, 3))
  Assert.Throws(() => Value.PipelineEncode(Identity, 3))
})
