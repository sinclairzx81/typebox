...
Test('Should Record 1', () => {
  ...
  const D = Value.PipelineDecode(A, { x: 1, y: 2 })
...
Test('Should Record 3', () => {
  ...
  const D = Value.PipelineDecode(Augment, { x: 1, y: 2 })
  const E = Value.PipelineEncode(Augment, D)
...
Test('Should Record 4', () => {
  ...
  const A = Type.Record(Type.Number(), NumberToString)
  const D = Value.PipelineDecode(A, { x: 1, y: 2, 0: 3 })
  const E = Value.PipelineEncode(A, D)
...
Test('Should Record 5', () => {
  const NumberToString = Type.Codec(Type.Number())
    ...
    .Encode((value) => parseFloat(value))
  const A = Type.Record(Type.Number(), NumberToString)
  Assert.Throws(() => Value.PipelineDecode(A, null))
  Assert.Throws(() => Value.PipelineEncode(A, null))
})
// ------------------------------------------------------------------
// Coverage
// ------------------------------------------------------------------
