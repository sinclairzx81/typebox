...
- Breaking
  ...
  - [Identifiers Removed for Composited Types](#Identifiers-Removed-for-Composited-Types)
...
Revision 0.26.0 enables auto reference resolution when type checking. Previous revisions of TypeBox required explicit passing of referenced types. Revision 0.26.0 will automatically resolve referenced types via a new internal `ReferenceRegistry`. Automatic reference resolution has been implemented for `TypeCompiler`, `Value` and `Errors` modules.
...
Revision 0.26.0 now returns a specialized `ValueErrorIterator` for calls to `.Errors(...)`. This type allows implementers to obtain the first error only. For all errors, use `for-of` enumeration or array spread syntax.
...
Revision 0.26.0 simplifies the representation for `TNever`. Previously version of TypeBox this type was represented as a illogical intersection of `allOf`. 0.26.0 updates the representation to be a `not` schema of type `any`.
...
<a name="Identifiers-Removed-for-Composited-Types"></a>

## Identifiers Removed for Composited Types

Revision 0.26.0 will remove internal `$id`'s from all non self-referential composited schemas. This change is to help ensure types remain immutable and uniquely identified. To have a composited schema refer to the original type, use `Type.Ref(...)`.
