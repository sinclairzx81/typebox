...
TypeBox now provides "runtime conditional types" (formally the `Conditional` module) as standard on `Type.*`. Additional updates in this revision automatic union and intersection unwrap, universal support for utility types, additional options for framework integrators and several ergonomic enhancements. This revision also carries out a number an internal refactorings to reduce the amount of submodule imports.
...
The `TypeGuard` is not imported via the `@sinclair/typebox` module. This move is due to the TypeBox compositor  now internally using the guard when constructing types.
