...
TypeBox now provides "runtime conditional types" (formally the `Conditional` module) as standard on `Type.*`. Additional updates in this revision include automatic type dereferencing, automatic union and intersection unwrap, universal support for utility types, automatic referenced type resolution, additional options for framework integration and several ergonomic enhancements.
...
Value.Check(R, [T])                                  // 0.25.0 - To check R, you needed to pass T

Value.Check(R)                                       // 0.26.0 - (new) T is now automatically resolved from R
```
