...
TypeBox now provides "runtime conditional types" (formally the `Conditional` module) as standard on `Type.*`. Additional updates in this revision automatic union and intersection unwrap, universal support for utility types, additional options for framework integrators and several ergonomic enhancements.
