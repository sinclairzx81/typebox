...
Revision 0.30.0 carries out several internal refactorings to reduce package and bundle sizes. This is largely an ongoing process with provisional work carried out across `type`, `value` and `compiler` modules.
...
Revision 0.30.0 provides a reference implementation for Transform types. There has been some interest from users to provide combinators similar to Zod's `.transform()` function to permit remapping values during `.parse()` operations. As TypeBox types do not have combinator functions and are just JSON Schema objects, introducing similar functionality without augmenting types or implementing `.parse()` on the types has proven to be particularily challenging. These challenges have prevented TypeBox from offering this feature as standard.

The reference implementation however supports augmenting types with codec functions. These functions allow values to be structurally encoded and decoded through the `.parse-like` functions `Encode()` and `Decode()`. TypeBox adopts the `io-ts` perspective for transforms, viewing the problem of transforming values as largely the role of dedicated codec system. As much of this functionality is considered high level, Transform types will not likely be added to TypeBox in the foreseeable future; rather users will be encouraged to implement transforms via external packages (using the reference implementation provided), or lifted verbatim into projects needing this functionality.

```typescript
import { Transform, Encode, Decode } from './transform' // from: examples/transform/trasform.ts

const Timestamp = Transform(Type.Number(), {       // Sets codec functions on a type
  decode: (value) => new Date(value),
  encode: (value) => value.getTime(),
})

const N = Type.Object({                            // Transform type can be nested within objects
  timestamp: Timestamp
})

const D = Decode(N, { timestamp: Date.now() })     // const D = { timestamp: Date }

const E = Encode(N, D)                             // const R = { timestamp: number }
```
