...
Use the `Value` module to perform common type operations on values. This module provides functionality to create, check and cast values into a given type. This module internally uses dynamic type checking. If requiring extremely fast type checking performance, consider using either Ajv the TypeBox [TypeCompiler](#compiler).
