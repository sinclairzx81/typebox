...
Reference Types can be used to reduce duplication when composing schemas. TypeBox provides supports referencing with the `Type.Ref(...)` and `Type.Box(...)` functions. The `Type.Ref(...)` function references into an existing type and `Type.Box(...)` provides a container for multiple referenceable types. To reference a type, you must specify an `$id` on the target type being referenced. The following shows referencing an existing `string` type.
...
The `Type.Box(...)` function provides a way to group related types under a common namespace. The following groups a set of math types under the namespace `Math3D` which are later referenced in the `Vertex` structure below.
