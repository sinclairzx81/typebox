...
TypeBox is built and tested against the Ajv validation library and also provides an optional high performance runtime validator with comparable performance built in for convenience. TypeBox can be used as a simple tool to build up complex schemas or integrated into RPC or REST services to help validate JSON data received over the wire. 
